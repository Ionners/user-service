...
func SetEnvConsulKV(v *viper.Viper) error {
...
func BindFromConsul(dest any, endPoint, path string) error {
	...
	if err != nil {
		logrus.Errorf("failed to add remove provider: %v", err)
		return err
	}

	err = v.ReadRemoteConfig()
	if err != nil {
		logrus.Errorf("failed to reada remote config: %v", err)
		return err
	}

	err = v.Unmarshal(&dest)
	if err != nil {
		logrus.Errorf("failed to rada unmarshal: %v", err)
		return err
	}
	err = SetEnvConsulKV(v)
	if err != nil {
